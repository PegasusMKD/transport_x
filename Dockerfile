...
#RUN pip install sklearn pandas numpy keras shapely tensorflow geopy

RUN cp /usr/bin/python3.6 /usr/bin/python3

RUN pip install sklearn
COPY src /transport_x

RUN SCONE_MODE=AUTO SCONE_HASH=1 SCONE_HEAP=1G SCONE_ALPINE=1		    \
	&& mkdir conf							    \
...
ENTRYPOINT unzip -o /in/dataset.json -d /in && python3 /transport_x/main.py
